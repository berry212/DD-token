...
def default_output_dir_for_dataset(dataset: str) -> str:
    if dataset == "skin-lesions":
        return "./artifacts/skin_lesions_token_resnet_classifier"
    return "./artifacts/token_resnet_classifier"
...
class TokenPseudoImageResNet(nn.Module):
    ...
    def _tokens_to_pseudo_image(self, token_ids: torch.Tensor) -> torch.Tensor:
        ...
        side = int(math.ceil(math.sqrt(float(seq_len))))
...
class LitTokenResNetClassifier(L.LightningModule):
    def __init__(self, cfg: TrainConfig, dm: TokenDataModule):
        super().__init__()
        self.cfg = cfg
        self.dm = dm

        self.model = TokenPseudoImageResNet(
...
def train(cfg: TrainConfig) -> None:
    ...
    model = LitTokenResNetClassifier(cfg, dm)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and evaluate a distilled-token pseudo-image ResNet classifier with Lightning."
    )
    parser.add_argument(
        "--dataset",
        type=str,
