...
def default_output_dir_for_dataset(dataset: str) -> str:
    if dataset == "skin-lesions":
        return "./artifacts/skin_lesions_mlp_baseline"
...
class LitPathMNISTMLP(L.LightningModule):
    def __init__(self, cfg: MLPTrainConfig, in_channels: int, num_classes: int):
        super().__init__()
        self.cfg = cfg
...
def train(cfg: MLPTrainConfig) -> None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train an MLP baseline on original datasets with Lightning.")
    parser.add_argument(
        "--dataset",
        type=str,
        default="pathmnist",
